...
const mainSetDefinitions = {
    ...
    'MAX_SPRINT': (energySystem, cssSecondsPer100, remainingDistanceForMainSet) => { // Was SP2
        ...
        // Calculate idealNumReps based on remaining distance
        let idealNumReps = (repDist > 0) ? Math.floor(remainingDistanceForMainSet / repDist) : 0;

        if (idealNumReps === 0 && repDist === 50 && remainingDistanceForMainSet >= sp2Distances[0]) {
            // Chosen 50yd but didn't fit (or fit 0 reps), try 25yd if enough distance for at least one 25
            repDist = sp2Distances[0]; // 25yd
            idealNumReps = (repDist > 0) ? Math.floor(remainingDistanceForMainSet / repDist) : 0;
        }

        // Define new absolute maximums, independent of sp2RepsMapping for capping.
        const absoluteMaxReps = (repDist === 50) ? 30 : 40; // e.g., 30x50=1500, 40x25=1000

        numReps = idealNumReps;
        numReps = Math.min(numReps, absoluteMaxReps);

        // Ensure a minimum of 1 rep if idealNumReps was 0 but there's enough distance for the selected repDist
        // (This primarily handles cases where remainingDistanceForMainSet is slightly less than repDist)
        if (numReps === 0 && remainingDistanceForMainSet >= repDist && repDist > 0) {
            numReps = 1;
        }

        // sp2RepsMapping is no longer used as a primary cap if remainingDistance allows for more, up to absoluteMaxReps.
        // It could be used as a suggestion or for other logic if needed, but not for capping here.
