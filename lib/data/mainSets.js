...
const mainSetDefinitions = {
    'ENDURANCE_BASE': (energySystem, cssSecondsPer100, remainingDistanceForMainSet) => {
        ...
        if (remainingDistanceForMainSet < 25) { // Absolute minimum distance
        ...
        // Adjust repDist if it's too large for the remaining distance
        if (remainingDistanceForMainSet < en1RepDist) {
            const possibleDists = en1Distances.filter(d => d <= remainingDistanceForMainSet);
            if (possibleDists.length > 0) {
                en1RepDist = possibleDists[possibleDists.length -1]; // largest possible distance
            } else { // if no standard distance fits, try to make a custom one or fail
                 if (remainingDistanceForMainSet >= 50) {
                    en1RepDist = Math.floor(remainingDistanceForMainSet / 50) * 50;
                    if (en1RepDist === 0) en1RepDist = 50; // ensure at least 50 if possible
        ...
        let numEn1Reps = (en1RepDist > 0) ? Math.floor(remainingDistanceForMainSet / en1RepDist) : 0;
        numEn1Reps = Math.min(numEn1Reps, 15);
        numEn1Reps = Math.max(numEn1Reps, 1); // Ensure at least 1 rep if distance allows for en1RepDist

        // Final check if the single rep (if numEn1Reps became 1) is too much
        if (numEn1Reps * en1RepDist > remainingDistanceForMainSet) {
             numEn1Reps = 0; // Cannot do even 1 rep of the chosen/adjusted en1RepDist
        }
    ...
    'THRESHOLD_SUSTAINED': (energySystem, cssSecondsPer100, remainingDistanceForMainSet) => {
        ...
        numEn2Reps = Math.min(numEn2Reps, 20);
    ...
    'MAX_SPRINT': (energySystem, cssSecondsPer100, remainingDistanceForMainSet) => {
        ...
        if (initialRepDist === 25 && has50) {
            const potentialReps50 = Math.min(Math.floor(remainingDistanceForMainSet / 50), 30);
